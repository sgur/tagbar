...
function! s:CheckForExCtags(silent) abort
    ...
    if s:shell_error || ctags_output !~# 'Exuberant Ctags'
        if !a:silent
...
function! s:ExecuteCtagsOnFile(fname, realfname, typeinfo) abort
    ...
    if s:shell_error || ctags_output =~ 'Warning: cannot open source file'
...
function! s:ExecuteCtags(ctags_cmd) abort
    ...
    let ctags_output = s:System(a:ctags_cmd)
...
function! s:System(cmd)
  ...
  if s:exists_vimproc
    ...
    let s:shell_error = vimproc#get_last_status()
  else
